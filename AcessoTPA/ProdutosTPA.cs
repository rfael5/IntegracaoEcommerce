...
public class ProdutosTPA
{
    ...
    public ProdutosTPA(PrincipalDbContext dbPrincipal)
    {
     ...
     public async Task<List<ProdutoEvento>> GetTodosProdutos()
    {
        var _query = @$"
            ...
            ORDER BY PRODUTO.ID      
        ";
        var produtos = await _dbPrincipal.ProdutosEvento.FromSqlRaw(_query).ToListAsync();
        return produtos;
    }

    public async Task<List<ProdutoEvento>> GetProdutosAcabados(
        QueryFilter filter,
