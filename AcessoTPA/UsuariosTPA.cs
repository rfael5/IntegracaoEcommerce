...
public class UsuariosTPA
{
    ...
    public UsuariosTPA(AppDbContext context, PrincipalDbContext dbPrincipal)
    {
        _context = context;
    ...
    public async Task<string> CriarCodigoClienteTray()
    {
        var maxCodigoCliente = await _dbPrincipal.CadastroUsuarioTPA.MaxAsync(usuario => usuario.codCadastro);
        var intCodigo = Convert.ToInt32(maxCodigoCliente) + 1;
        var novoCodigo = intCodigo.ToString("D6");
        return novoCodigo;
    }

    public async Task<string> CriarPkCadastroTray()
    {
        var ultimoId = await _dbPrincipal.Database
        .SqlQueryRaw<int>("SELECT MAX(ID) AS Value FROM TPACADASTRO")
        .FirstAsync();
        var ultimoIdMaisUm = ultimoId + 1;
        var novoPkCadastro = $"       {ultimoIdMaisUm}";
   ...
   public async Task<TpaCadastroDTO> CadastrarUsuarioTray(DadosCliente dadosCliente)
    {
        ...
        _dbPrincipal.CadastroUsuarioTPA.Add(novoUsuario);
        await _dbPrincipal.SaveChangesAsync();
        Console.WriteLine("adicionado");
        Console.WriteLine(novoUsuario.id);
        await _dbPrincipal.Entry(novoUsuario).ReloadAsync();
        return novoUsuario;
    }
    ...
    public async Task<string> CadastrarEnderecoUsuarioTray(string idUsuario, DadosEntrega dadosEntrega)
    {
        ...
        _dbPrincipal.Enderecos.Add(enderecoUsuario);
        await _dbPrincipal.SaveChangesAsync();
        await _dbPrincipal.Entry(enderecoUsuario).ReloadAsync();
        return enderecoUsuario.pkEndereco;
    }
    ...
    public async Task CadastrarContatoTray(TpaCadastroDTO cadastro)
    {
        ...
        _dbPrincipal.Contatos.Add(contatoUsuario);
        await _dbPrincipal.SaveChangesAsync();
        await _dbPrincipal.Entry(contatoUsuario).ReloadAsync();
    }
    ...
    private async Task<UserKeys> ClienteCadastradoTray(string _cnpjcpf)
    {
        var cadastro = await _dbPrincipal.CadastroUsuarioTPA.Where(usuario => usuario.cnpjCpf == _cnpjcpf).SingleOrDefaultAsync() ?? null;
        string _pkEndereco;
        if(cadastro == null)
        {
            return new UserKeys();
        ...
        {
            _pkEndereco = await _dbPrincipal.Enderecos.Where(end => end.idxTabela == cadastro.pkCadastro).Select(end => end.pkEndereco).SingleOrDefaultAsync() ?? "";
        }
        return new UserKeys(){pkCadastro = cadastro.pkCadastro, pkEndereco = _pkEndereco};
    }
}